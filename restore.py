...
if config["db"]["type"] == "postgres":
    ...
    print(   "mysql --host=%s --user=%s --password=%s %s < %s/%s" % (config["db"]["host"], config["db"]["user"], config["db"]["password"], config["db"]["database"], tmpPath, filename))
    os.popen("mysql --host=%s --user=%s --password=%s %s < %s/%s" % (config["db"]["host"], config["db"]["user"], config["db"]["password"], config["db"]["database"], tmpPath, filename))
else:
